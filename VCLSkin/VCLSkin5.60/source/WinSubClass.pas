...
implementation
uses ImgUtil,WinSkinForm,winskindlg,CheckLst;
...
begin
    ...
    kind :=0;
{$IFDEF VER210}
//    if (control<>nil) and TS then
//       caption:= NonClientMetrics.cbSize := SizeOf(NonClientMetrics)-SizeOf(NonClientMetrics.iPaddedBorderWidth);
{$endif}
...
begin
   ...
   while ( hCtrl<>0 ) do begin
      ...
      if (s='msctls_updown32') or (pos('spin',s)>0) or (pos('updown',s)>0)
        { or (pos('btn',s)>0)} then begin
          updown:=Tskinupdown.create(self.owner);
