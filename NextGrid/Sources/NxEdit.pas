{
  ...
  $id:NxEdit.pas 3/19/2005 3:30:00 bn
}
...
{$HINTS ON}
  ...
  TNxMemoInplaceEdit = class(TNxCustomEdit)
  private
...
begin
  if Assigned(FGlyph) and not FGlyph.Empty
    and PtInRect(Rect(0, 0, FGlyph.Width, Height), Point(X, Y))
      then DoGlyphClick;
end;
...
begin
  inherited;
  if FEditInfo.Visible then FEditInfo.Invalidate;
end;
...
begin
  ...
  SetWindowPos(FButton.Handle, HWND_TOP, ClientWidth - FButton.Width,
    0, FButton.Width, Self.ClientHeight, SWP_SHOWWINDOW or SWP_NOREDRAW);
  FButton.Repaint;
end;
...
//  if AsString <> '' then
  begin
    ...
    begin
      case Key of
        VK_LEFT: Date := IncDay(Date, -1);
        VK_RIGHT: Date := IncDay(Date, 1);
...
(*  GetKeyboardState(State) ;
  CtrlPressed := ((State[vk_Control] And 128) <> 0);

  if CtrlPressed then
  begin
    Delta := 10 * (SelStart - length(TextAfter));
  end;
*)
